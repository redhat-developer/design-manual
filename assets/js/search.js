(function( $ ) {
    ...
    (lunr.stemmer = (function () {
      ...
      return function (n) {
        ...
        if (
          ...
          (i = m[1]),
            (f = RegExp(a)),
            f.test(i) && ((n = i), (f = /(at|bl|iz)$/), (d = RegExp("([^aeiouylsz])\\1$")), (v = RegExp("^" + o + r + "[^aeiouwxy]$")), f.test(n) ? (n += "e") : d.test(n) ? ((p = /.$/), (n = n.replace(p, ""))) : v.test(n) && (n += "e"));
        }
        if (((p = /^(.+?)y$/), p.test(n))) {
          var m = p.exec(n);
  ...
  $.fn.camelHunter 	= function( options ) {

     //Here we use jQuery's extend to set default values if they weren't set by the user
      var opts 		= $.extend( {}, $.fn.camelHunter.defaults, options );
      if( opts.results )
      {
  ...
  $.fn.camelHunter.defaults = {
    results           : false,
    rss               : "/rss-alternate",
    onKeyUp           : true,
    result_template   : "<li class='pf-c-data-list__item' aria-labelledby='results'><div class='pf-c-data-list__item-row'><div class='pf-c-data-list__item-content'><div class='pf-c-data-list__cell'><div><b id='page-title'><a href='{{link}}' aria-label='{{title}}' style='font-size: 1.2em;'>{{title}}</a></b><div><b>Status: </b><span style='text-transform: capitalize;'>{{status}}</span></div><div><b>Section</b>: {{active_nav}}</div></div></div><div class='pf-c-data-list__cell'><b>Description</b><p>{{description}}</p></div></div></div></li>",
    info_template     : "<h2>Number of pages found: {{amount}}</h2>",
    ...
    /*	Here we load an rss feed, parse it and load it into the index.
      ...
      $.get(rssURL,function( data ){
          ...
          for (var i = 0; posts && i < posts.length; i++) {
            ...
            section : post.find('section').text(),
            active_nav : post.find('active_nav').text(),
